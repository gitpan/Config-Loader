...
ok($c = Config::Loader->new(path => $path, skip => qr{skip_\d}),
   'new - skip - regex'
   );
$C=$c->();
...
ok($c = Config::Loader->new(path => $path, skip => [qr{skip_1}, qr{skip_2}]),
   'new - skip - regexen'
   );
$C=$c->();
...
ok($c = Config::Loader->new(path => $path, skip => {'skip_1' => 1,'main.skip_1' => 1}),
   'new - skip - hash'
   );
$C=$c->();
...
ok($c = Config::Loader->new(path => $path, skip => sub {
     my ($self,$filename) = @_;
...
ok($c = Config::Loader->new(path => $path),
...
ok($c = Config::Loader->new(path => $path, load_as => qr/(.*)-\(local\)/),
   'new - load_as - regex'
   );
$C=$c->();
...
ok($c = Config::Loader->new(path => $path, is_local => qr{override}),
   'new - is_local - regex - ..'
   );
...
ok($c = Config::Loader->new(path => $path, is_local => {override => 1}),
   'new - is_local - hash - ..'
   );
...
ok($c = Config::Loader->new(path => $path, is_local => sub { return $_[1] eq 'override'}),
   'new - is_local - sub - ..'
   );
...
ok($c = Config::Loader->new(path => $path, debug => 1),
   'new - debug'
   );
